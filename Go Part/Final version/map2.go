...
func main() {
  ...
  // channel qui communiquera les jobs
  ...
  // Consumme already applies the DBSCAN on the partitions
  mutex.Wait()
...
func DBscan(coords []LabelledGPScoord, MinPts int, eps float64, offset int) (nclusters int) {
   ...
   */
   ...
   for _,point:= range coords{
    ...
    nclusters += 1 //Update the counter
    label := nclusters + offset //Create the cluster label
    point.Label = label //assign Label to point

    //assign label to neighbors of point
    for _,pointN:= range neighbors{
      if pointN.Label == -1{ pointN.Label = label }

      if pointN.Label != 0{continue}

      pointN.Label = label
      newNeighbors := rangeQuery( pointN,coords, eps)

      //Extands cluster if pointN has enough neighbors
      if len(newNeighbors)>= MinPts{
        for _,newPoint:= range newNeighbors{
...
//My own methods_________________________________________________________________
...
//Consomme "cree" un consommateur qui reçoit les index i et j
// identifiant une partition dans grid
//Ce consommateur procede ensuite a un DBscan de la partition renseignee
func consomme(jobs chan int, done *sync.WaitGroup, grid [4][4][]LabelledGPScoord) {
	for {
		i, more := <-jobs
    j, more := <-jobs
